...
class Maxout(NonLinear):
    debugname = 'maxout'
    LayerTypeName = 'Maxout'
    yaml_tag = u'!Maxout'

    def __init__(self, function_size=10):
        super(Maxout, self).__init__()
        self.funcCount = function_size
    ...
    def fillToObjMap(self):
        objDict = super(Maxout, self).fillToObjMap()
        return objDict

    def loadFromObjMap(self, tmap):
        super(Maxout, self).loadFromObjMap(tmap)

    @classmethod
    def to_yaml(cls, dumper, data):
        obj_dict = data.fillToObjMap()
        node = dumper.represent_mapping(Maxout.yaml_tag, obj_dict)
        return node
